  ...
  for (let i = 0; i < books.length; i++) {
    ...
    output += `
      <div class="book-container" data-id="${book.id}">
        ...
        <p>Year: ${book.year}</p>
        <img src="" data-src="${book.imageLink}" alt="${book.title}" class="lazy-image">
...
// Lazy loading - Load images when in view
...
function displaySuggestions(filteredBooks) {
  const suggestions = filteredBooks
    .map(book => `<li>${book.title}</li>`)
    .join("");

  suggestionsContainer.innerHTML = `<ul>${suggestions}</ul>`;
  suggestionsContainer.style.display = suggestions ? "block" : "none";
}

function displayFilteredBooks(filteredBooks) {
  const filteredBookTitles = filteredBooks.map(book => book.title.toLowerCase());

  const allBooks = document.querySelectorAll(".book-container");
  let rowCounter = 0;
  allBooks.forEach(book => {
    const title = book.querySelector("h3").textContent.toLowerCase();
    const display = filteredBookTitles.includes(title) ? "block" : "none";
  ...
  rowCount.textContent = `Rows Shown: ${rowCounter}`;
}
...
suggestionsContainer.addEventListener("click", e => {
  if (e.target.tagName === "LI") {
    ...
    searchBar.value = selectedBookTitle;
    suggestionsContainer.style.display = "none";
    handleSearch();
  }
});
